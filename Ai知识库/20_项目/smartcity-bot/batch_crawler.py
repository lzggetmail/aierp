...
def batch_crawl_last_year():
    ...
    print(f"📅 采集时间范围：{one_year_ago.strftime('%Y-%m-%d')} 至今\n")
        ...
        try:
            if hasattr(result, 'date') and result.date:
                ...
                if pub_date < one_year_ago:
                    print(f"   ⏭️  跳过（发布于 {result.date}，超过一年）")
                    filtered_count += 1
                    continue
        except:
            # 如果无法解析日期，保留该内容
    ...
    print("📊 采集统计")
    print("=" * 60)
    print(f"总采集数: {len(results)} 条")
    print(f"时间过滤: {filtered_count} 条（超过一年）")
    print(f"有效内容: {stored_count} 条")
    print(f"跳过内容: {len(results) - stored_count - filtered_count} 条")
    print("\n✅ 批量采集完成！")
    print(f"\n💡 输入 /kb stats 查看知识库统计")
